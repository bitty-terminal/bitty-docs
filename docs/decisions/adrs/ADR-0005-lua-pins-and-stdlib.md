...
and the [Lua Runtime RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/lua-runtime-rfc.md) without
...
- Deciders: project initiator (DEC-001), security-auditor persona (audit gate
  per [Lua Runtime RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/lua-runtime-rfc.md) security review
...
- Successor-direction note (ADR 0012, 2026-09-20): the plugin-VM successor path
  is now Phodopus, a sandbox-first fork of `piccolo`, per
  [ADR 0012](ADR-0012-phodopus-runtime.md). This refinement adds direction only;
  the accepted `mlua`/Lua 5.4.x and `piccolo 0.3.3` pins, allowlist, cadence,
  and audit gates below remain accurate and in force until an implementing task
  migrates `bitty-lua`.
...
| piccolo                | `0.3.3` retained from CTX-0040 `d67a65b`                                                                                         | `piccolo = "=0.3.3"` exact no caret in `bitty-lua/Cargo.toml`                                                     | Plugin VM; pure-Rust `#![forbid(unsafe_code)]`; deps `gc-arena 0.5.3` and `sptr 0.3.2` pinned transitively via `Cargo.lock`. Successor direction: Phodopus per [ADR 0012](ADR-0012-phodopus-runtime.md); this pin remains accurate until a migration task lands |
...
| Pure piccolo for both VMs                          | Isolation Resource RFC watch-list evolution piccolo 0.3.3 in CTX-0040 | Not P0 — plugin VM uses piccolo now; Config VM stays mlua per ADR 0004. Re-evaluate when piccolo Lua 5.4 compat plus diagnostics match `bitty config check` and measurement infra proves PB-1 and PB-2 headroom. Refined 2026-09-20: the plugin-VM successor direction is Phodopus per [ADR 0012](ADR-0012-phodopus-runtime.md) |
...
- [ADR 0012](ADR-0012-phodopus-runtime.md) — Phodopus Runtime as the Lua Successor Path, accepted 2026-09-20 (successor-direction refinement only)
- [ADR 0004](ADR-0004-upstream-dependencies.md) — Upstream Dependency Set Accepted P0 baseline piccolo watch-list
- [Lua Runtime RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/lua-runtime-rfc.md) — Accepted 2026-08-27 sandbox stdlib baseline module search diagnostics Open items
- [Isolation Resource RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/isolation-resource-rfc.md) — RC-1 and RC-2, FS-1 through FS-9, attack cases AT-IR-004 through AT-IR-007
...
7. **Docs sync:** this ADR appears in [decision register](../index.md) and [ADR index](README.md) plus `bitty-plugins/specifications/lua-runtime-rfc.md` Open items updated plus [open-question register](../open-questions.md) OQ-030 row to Accepted ADR 0005 in the same PR per register close rule.
