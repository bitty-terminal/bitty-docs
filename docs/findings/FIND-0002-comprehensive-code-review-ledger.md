...
`FIND-0002` records the comprehensive code review of the Bitty terminal
workspace conducted on 2026-09-06 under task `CTX-0121`. The original scope
covered the 18 crates listed below and examined memory safety, cryptographic
and sandbox boundaries, terminal protocol fidelity, resource limits, and
concurrency hazards. This is a historical review record, not a current
workspace inventory; the synchronized workspace is 21 crates at `679f12f`.
Observations were compared with non-normative external terminal patterns;
no local reference checkout is required to use this page.

| Field              | Value                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scope              | Historical review scope: 18 crates in `bitty/crates/` at the 2026-09-06 review revision (`bitty-core`, `bitty-vt`, `bitty-term-state`, `bitty-pty`, `bitty-render`, `bitty-platform`, `bitty-ui`, `bitty-runtime`, `bitty-rich`, `bitty-lua`, `bitty-plugin-host`, `bitty-package`, `bitty-ipc`, `bitty-agent`, `bitty-config`, `bitty-app`, `bitty-perf`, `bitty-compat-lab`). The current workspace is 21 crates at `679f12f`. |
| Evidence date      | 2026-09-06                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Source task        | `bitty-docs/CTX-0121`                                                                                                                                                                                                                                                                                                                                                                                                            |
| Affected contracts | [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md), [P0 Acceptance Criteria](../security/p0-acceptance-criteria.md), [Threat Model](../security/threat-model.md)                                                                                                                                                                                           |
...
| `CR-COMPAT-01` | FIXED   | `bitty` [`fd8a71a`](https://github.com/bitty-terminal/bitty/commit/fd8a71a4ccefc69272503cc3f264176bceff5d06) (`CTX-0206`, PR #354, correct the workspace reference-directory name) plus [`0e65f85`](https://github.com/bitty-terminal/bitty/commit/0e65f85d9cf153a2ba3824bfe2c81586015941d1) (`CTX-0211`, PR #366, regenerate baselines post v2/v3 hash drift) | Comparator candidate discovery uses the repository's configured singular reference directory; baselines were regenerated after the v2/v3 hash drift.                                                                      |
...
| `CR-COMPAT-01` | MEDIUM   | `bitty/CTX-0206` + `CTX-0211`   | Hardcoded snapshot-directory name breaks dump discovery after the workspace rename      | `fd8a71a` PR #354 + `0e65f85` PR #366, FIXED |
...
| `CR-COMPAT-01` | **MEDIUM**   | `bitty-compat-lab/src/compare.rs:56, 60`                                                 | Tooling / Compat   | Hardcoded snapshot-directory name breaks dump discovery after the workspace rename        | Differential comparator cannot locate baseline snapshots on disk                                 |
...
- **Reference Comparison**:
  - **Alacritty** (`alacritty_terminal/src/term/cell.rs`): Uses a bounded zero-width storage design.
  - **Xterm** (`charproc.c`): Applies a bounded combining-character budget.
...
### 3. `CR-COMPAT-01`: Outdated snapshot candidate paths after directory rename

- **Location**: `bitty/crates/bitty-compat-lab/src/compare.rs:56, 60`
- **Root Cause**: The comparator used a plural directory name in its
  workspace-root join after the repository convention had changed to the
  singular form. The defect was the hard-coded name, not the location of any
  checkout or evidence store.
- **Remediation**: Use the repository's configured reference-directory name
  when discovering differential candidates, and regenerate baselines after
  the snapshot-hash change.
...
By comparing the Bitty codebase with non-normative external terminal
implementations, the following architectural enhancements are identified:

### 1. Clipboard OTP & Granular Paste Safety (Ghostty Pattern)

- **Reference**: Ghostty terminal clipboard and clipboard-grant implementations
...
- **Reference**: Ghostty terminal graphics image and storage implementations
- **Architecture**:
...
- **Reference**: Hyprland's tiled Dwindle layout algorithm
...
- **Reference**: Waybar module and bar implementations
...
- **Reference**: Hermes Agent tool registry and MCP tool implementations
- **Architecture**:
...
- **Reference**: Neovim window configuration and extmark implementations
