...
| Status             | Open (2 of 16 fixed, 14 open per 2026-09-06 triage under `CTX-0125`; see Remediation triage)                                                                                                                                                                                                                            |
...
only disposition changes are recorded here.
...
### Open (14, each owned by one `bitty` task)

All 14 items below remain Open. Each row names the owning `bitty` task
(`CTX-0197` through `CTX-0210`, all `planned` at triage time) as a read-only
reference. No `bitty` state is changed from this document.

| ID             | Severity | Owning `bitty` task (read-only) | Finding summary                                                                         |
| -------------- | -------- | ------------------------------- | --------------------------------------------------------------------------------------- |
| `CR-PKG-02`    | CRITICAL | `bitty/CTX-0197`                | Silent TOFU pin check bypass in `TrustMode::Signed` when `candidate_identity` is `None` |
| `CR-RICH-02`   | HIGH     | `bitty/CTX-0198`                | Empty or relative resource roots grant universal read via `starts_with("")`             |
| `CR-PKG-01`    | CRITICAL | `bitty/CTX-0199`                | `prune` skips the current generation without replacing the removal candidate            |
| `CR-PKG-03`    | CRITICAL | `bitty/CTX-0200`                | Manifest capability parser diverges from the host closed-set validator                  |
| `CR-RICH-01`   | HIGH     | `bitty/CTX-0201`                | Public `RichBlock` fields bypass AST node count, depth, and text byte limits            |
| `CR-RENDER-01` | HIGH     | `bitty/CTX-0202`                | Headless `present_draw_list` allocates without a `MAX_SURFACE_BYTES` bound              |
| `CR-IPC-01`    | HIGH     | `bitty/CTX-0203`                | `metadata` follows symlinks in socket directory and bound socket attestation            |
| `CR-RT-01`     | HIGH     | `bitty/CTX-0204`                | OSC 52 clipboard read discards text instead of replying base64                          |
| `CR-TERM-02`   | MEDIUM   | `bitty/CTX-0205`                | Unbounded cursor and tab loop iterations on large counts                                |
| `CR-COMPAT-01` | MEDIUM   | `bitty/CTX-0206`                | Hardcoded `recordings/` paths break dump discovery after the `recording/` rename        |
| `CR-APP-01`    | MEDIUM   | `bitty/CTX-0207`                | Unknown CLI flags starting with `-` are treated as program names                        |
| `CR-TERM-01`   | MEDIUM   | `bitty/CTX-0208`                | Zero-width scalars and combining characters are silently dropped                        |
| `CR-UI-01`     | MEDIUM   | `bitty/CTX-0209`                | Rigid Dwindle BSP splitting without aspect-ratio heuristics                             |
| `CR-LUA-01`    | LOW      | `bitty/CTX-0210`                | Fuel and step limits are checked only at slice boundaries                               |
...
Owning `bitty` tasks for every open item are listed under
[Remediation triage](#remediation-triage-2026-09-06-ctx-0125); `CR-PLAT-01`
and `CR-UI-02` are already FIXED there and need no new work.
