...
description: Records the accepted pixel-art-first direction for the Bitty mascot and derived shared visual assets, the completed high-resolution reference art, and the production and rendering constraints that apply to future asset work.
category: product
audience: mixed
...
- Terminal-side images follow terminal image protocols: Kitty Graphics Protocol
  as the primary path, Sixel for compatibility, with placement separated from
  the image payload.
...
local research note `recording/research/020.md` (archived as `020.md.completed`),
which covers 88×31 web badges, pixel-art mascot systems, terminal cell geometry
versus image pixels, and image-protocol and zoom semantics. The research note is
workspace scratch material; this document is the canonical record.
