...
**Pre-alpha / Engineering Milestones M1-M8** at 21 crates (`23c3eb6`) with lifecycle
...
- The `bitty/` workspace is spine-complete (twenty-one members in
  `bitty/Cargo.toml` as of 2026-09-19 `23c3eb6`): `bitty-vt`, `bitty-term-state`,
...
## Workspace structure (Pre-alpha / Engineering Milestones M1-M8, 21 crates `23c3eb6`)

The workspace is spine-complete as of 2026-09-19 (`23c3eb6`). The accepted
topology is
...
- Verification of `bitty-package` (lifecycle accepted, signatures draft) and
  ...
  through `23c3eb6`) from `Implemented` to `Verified` per risk evidence RFC
