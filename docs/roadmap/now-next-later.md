...
- **Horizon anchor:** [Release Ladder](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md) stage
  **Pre-alpha / Engineering Milestones M1-M8** at `bitty` `679f12f` (`21 crates`, `54 OQs`
  ...
  `Implemented`-only and the `v0.0.20` and `v0.0.21` plugin-runtime,
  Kitty-graphics, decoration, config-matrix, release, and plugin-CLI waves plus
...
- **Lifecycle:** `Draft -> Experimental Implementation -> Accepted -> Verified -> Compatible -> Release-ready`
  ...
  (crate) per the [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md) RS-1
...
[Panel Extensibility Vision](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/panel-vision.md) rename note and
the [DevTools RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/devtools-rfc.md) Amendment A2.
...
Post-`29772a3` `bitty` origin `main` advanced through `bea338d` (previous
`29772a3`, CTX-0180 refresh) to `23c3eb6` (previous `bea338d`, CTX-0233
refresh): the `v0.0.20` release (`d9f5b49`, 2026-09-11) plus post-release
hardening added `bitty-test-support` (CTX-0267), `bitty-panels` (CTX-0438),
and `bitty-test-vm` (CTX-0507) — a 21-crate workspace — together with the
...
- **Dependencies:** [Rich Presentation RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/rich-presentation-rfc.md) hyperlink transport, [Platform Support Tiers](../decisions/adrs/ADR-0002-platform-support-tiers.md) native handler policy, ladder `v0.8` rich slice.
...
- **Dependencies:** [Plugin Platform RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/specifications/plugin-platform-rfc.md) (OQ-011/012/013), [Lua Runtime RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/lua-runtime-rfc.md) (OQ-009), ladder `v0.4` Lua and `v0.5` Plugin API.
...
- **Dependencies:** [Isolation Resource RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/isolation-resource-rfc.md) RC-1..RC-5 and FS-1..FS-9 (OQ-014 Accepted 2026-08-28), ladder `v0.5`/`v0.7`.
...
- All five Now items preserve all five release-blocking invariants cited in the
  [Release Ladder](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md) and do not downgrade any
...
- **Ladder slice:** [Release Ladder v0.8](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md#candidate-maturity-ladder-from-proposed-delivery-sequence)
...
- **Dependencies:** Now-2 budgets, [Rich Presentation RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/rich-presentation-rfc.md),
  [Architecture overview](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/architecture/overview.md) Terminal/View/Layout
...
- **Ladder slice:** [Release Ladder v0.9](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md#candidate-maturity-ladder-from-proposed-delivery-sequence)
...
- **Ladder slice:** [Release Ladder v1.0](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md#candidate-maturity-ladder-from-proposed-delivery-sequence)
...
- **Deferred items:** `bittyd` and remote UI are **post-v1.0** candidates per
  ...
  The [Proposed Delivery Sequence](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/proposed-delivery-sequence.md)
...
- **Candidate direction:** the future Panel Extensibility Vision document
  ...
  is not a PTY. The
  [Workspace Compositor](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/workspace-compositor.md) remains
...
- **Candidate scope:** after v1.0, a future Panel RFC may define Panel
  ...
  browser, or AI workflows. The accepted [IPC and Agent RFC](https://github.com/bitty-terminal/bitty-ai-docs/blob/main/specifications/ipc-agent-rfc.md)
...
`main` `3f5ed24` on 2026-09-09. The comparison material was treated as
untrusted input, was not executed, and no vendor documentation is copied into
this repository. Survey cells marked unknown are honest evidence gaps; rows
marked **evidence-limited** (accessibility, IME, font-zoom cross-vendor rank,
file-watcher reload) need one more source pass before their rank is treated
as fixed. Post-survey `bitty` movement is recorded below only as
`Implemented` status; it does not update the snapshot, milestone frame, or
maturity stated above.
...
[Core and Plugin Boundaries](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/architecture/core-boundaries.md) M1 Hardening
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Multiple OS windows              | Absent; no `NewWindow` action. All six references support multiple OS windows (Alacritty via `msg create-window`). | Core mechanism: [Workspace Compositor](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/workspace-compositor.md) `Instance -> Window` hierarchy and `bitty-platform` window ownership. The [Single-Window Vertical Slice](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/vertical-slice-acceptance.md) explicitly excludes multi-window. Post-survey workspace ops (`bitty` #433 `227ca3a`, CTX-0257) are in-app workspace switching, not OS windows. |
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tabs + tab bar                                        | Absent; `Stack` layout is tab-like only.                                                                               | None. `227ca3a` (#433, CTX-0257) adds workspace ops and a workspaceline overlay, not a tab bar.                                                                 | Plugin policy over Core primitives per [Core and Plugin Boundaries](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/architecture/core-boundaries.md) and `bitty-terminal.tabs` in the [plugin roadmap](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/product/plugin-roadmap.md); layout stays with the compositor.                                                                                                 |
...
| Kitty keyboard protocol                               | Absent.                                                                                                                | None.                                                                                                                                                           | Core input mechanism: [Input and Pointer Contract](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/input-pointer-rfc.md) (Draft) opt-in negotiation; [Compatibility Milestone RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/compatibility-milestone-rfc.md) keeps it a progressive enhancement rather than M1-required.                                                        |
| Vi / copy mode                                        | Absent; mouse selection exists.                                                                                        | None.                                                                                                                                                           | Selection and input contract; keyboard selection mode is unowned.                                                                                                                                                                                                                                                                                                                                                                                   |
| Hints / quick-select                                  | Absent.                                                                                                                | None.                                                                                                                                                           | [Semantic Terminal RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/semantic-terminal-rfc.md) hints unit (draft) covers semantic targets; no mouse quick-select UX contract.                                                                                                                                                                                                                                     |
...
| Command palette                                       | Absent; opt-in Command Composer only.                                                                                  | None.                                                                                                                                                           | Plugin-owned palette and picker over overlay primitives per Core and Plugin Boundaries; `bitty-terminal.palette` is bundled disabled in the [Default Distribution RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/default-distribution-rfc.md), with overlay types in the [Panel Runtime Pre-Study](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/panel-runtime-pre-study.md). |
...
| Per-codepoint font fallback                           | Absent (crossfont only).                                                                                               | None.                                                                                                                                                           | [Text and Rendering RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/text-rendering-rfc.md) (Draft) font discovery and coverage-driven fallback candidate scope.                                                                                                                                                                                                                                                 |
| Grapheme-cluster measurement                          | Partial: grapheme segmentation and bounded combining storage exist; `char_cell_width` remains a compact approximation. | None.                                                                                                                                                           | Text and Rendering RFC UAX #29 grapheme-to-cell replacement; full cluster measurement and shaping remain candidate work outside the vertical slice.                                                                                                                                                                                                                                                                                                 |
| Kitty graphics present path                           | Work in progress (CTX-0248 at survey).                                                                                 | Landed `0cc82de` (#425), `bd15d94` (#427), and follow-ups `3d45924` (#465), `6d4aa2f` (#463), `18c333a` (#467), `fdd9e28` (#505); `Implemented` not `Verified`. | [Rich Presentation RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/rich-presentation-rfc.md); already in flight, do not re-plan.                                                                                                                                                                                                                                                                                |
| User-facing scriptability (window, tab, pane control) | Devtools socket only; consent-gated.                                                                                   | `bitty ctl` workspace verbs landed `227ca3a` (#433, scope-gated `view.manage` / `terminal.manage`) and `8b987a0` (#457); `Implemented` not `Verified`.          | [IPC and Agent RFC](https://github.com/bitty-terminal/bitty-ai-docs/blob/main/specifications/ipc-agent-rfc.md) and [DevTools RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/devtools-rfc.md); Later-2 `v0.9`.                                                                                                                                                                                                  |
...
| Live config reload (file watcher)                     | Reload module exists; watcher unverified; **evidence-limited**.                                                        | `5afb8a2` (#490, CTX-0295) adds reload knob-effect tests; watcher evidence remains unproven.                                                                    | [Configuration Model RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/configuration-model-rfc.md) `Live` reload semantics.                                                                                                                                                                                                                                                                                       |
...
| Bidi / RTL shaping                                                                                                                                                               | Partial: split mechanics and grapheme segmentation exist; full UAX #9 reordering and shaping remain absent. | Text and Rendering RFC bidi-reordered present candidate scope.                                                                                                                                                       |
...
the [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md) taxonomy where
...
| Gap register (CTX-0251)    | `docs-curator` for the register; per-row owning RFC or ADR owners once assigned                                                               | Independent product and architecture review before any rank becomes accepted scope; security review for trust-boundary rows | Current implementation evidence and the owning contract named per row; the survey comparison remains candidate | Low — survey triage only; evidence-limited rows need another source pass                    |
...
Every success claim below maps to the taxonomy in the [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md)
...
- No calendar dates, no release train commitment, and no website publication
  ...
  [Release Ladder](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md) candidate ladder, not dates.
...
- No bypass of the [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md)
...
- No claim that `bitty` implements Later candidates such as `bittyd`, remote
  ...
  [Product vision](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/vision.md) non-goals and the
  [Proposed Delivery Sequence](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/proposed-delivery-sequence.md)
...
- When a Later slice moves from `Implemented` to `Verified`, update its status
  in the [Release Ladder](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/release-ladder.md) and the linked
...
- The [Shared-conversation coverage](../sources/chatgpt-share-coverage.md)
  route-item matrix and the [Proposed Delivery Sequence](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/proposed-delivery-sequence.md)
...
- [Risk Evidence RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/risk-evidence-rfc.md) — RS-1..RS-7 lifecycle, traceability, and entry checklist for `OQ-025`.
...
- [Website Delivery RFC](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/specifications/website-delivery-rfc.md) — pinned `src/content/docs-revision.json` and `/docs/<version>/<path>/` for website publication; this roadmap is not published via that path while `website_publish: false`.
...
- [Single-Window Vertical Slice Acceptance Plan](https://github.com/bitty-terminal/bitty-terminal-docs/blob/main/product/vertical-slice-acceptance.md) — candidate acceptance contract for the first real one-window terminal slice (CTX-0109, draft, depends on CTX-0107/0108; PB-1..PB-7, Tier 1/2, explicit exclusions; no code authorized until review gates).
- CTX-0251 terminal feature survey — read-only comparison of six modern terminals against `bitty` `3f5ed24` (2026-09-09); the comparison was untrusted, never executed, and is not copied into this repository. The [gap register](#gap-register-modern-terminal-feature-survey-candidate) above is the canonical, self-contained summary and is superseded only by a reviewed successor survey.
