...
| [TerminalRegistry and View Lifecycle Contract](terminal-registry-view-lifecycle-rfc.md) | OQ-005, OQ-007 (lifecycle refinement)       | Draft    |
...
| [TerminalRegistry and View Lifecycle Contract](terminal-registry-view-lifecycle-rfc.md) | TerminalId/ViewId, focus, resize                 | Draft  | `c0aadd2`/`a8735d0` at `a8735d0` (view-rect -> PTY, `write_replies`)                        | P0 — slice registry                     |
...
Review note (2026-08-31, reconciled per CTX-0116, `bitty` `a8735d0` chain `d4d75e9 -> c0aadd2 -> 7e3104d -> a8735d0`, `R-004` `Open`, `R-005`/`R-006`/`R-007` `Mitigated`, experimental `c0aadd2`/`7e3104d`/`a8735d0` `Implemented` not `Verified`): the Configuration Model RFC targeting OQ-010,
...
crate docs; experimental implementations `c0aadd2`/`7e3104d`/`a8735d0` do not imply `Accepted`/`Verified`. The twenty accepted artifacts (Performance Budget RFC OQ-001, ADR-0002
...
remain `Accepted` as recorded in the [decision register](../decisions/index.md); seven drafts remain `Draft` (not `Accepted`/`Verified`) with experimental code only for Input/Text/Registry.
