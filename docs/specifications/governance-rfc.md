...
| Grouping directory    | A local path that only groups repositories, never a Git repository itself: the umbrella root, the `bitty-plugins/` grouping, and the `bitty-ai/` grouping (whose only child today is the `bitty-mcp` checkout). |
