...
| Command palette user entry                     | implemented-only (unwired) | Palette panel exists over the Panel Runtime (`crates/bitty-runtime/src/palette.rs`) and `bitty-terminal.palette` is bundled-disabled (`bitty-docs/docs/product/plugin-roadmap.md:186`); no keybinding reaches it.                                                                        | kitty `docs/kittens/command-palette.rst`; wezterm `docs/features.md` (command palette action); ghostty `src/config/Config.zig` search colors |
...
| Terminal terminfo entry                        | partial (by design today)  | The PTY advertises `TERM=xterm-256color` and `TERM_PROGRAM=bitty` (`crates/bitty-pty/src/builder.rs:49-57`); no installed `bitty` terminfo entry ships yet.                                                                                                                              | kitty ships `kitty/terminfo.py`; xterm `ctlseqs.txt` is the baseline                                                                         |
