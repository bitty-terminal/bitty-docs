...
# bitty-docs checkout (the trigger is the merge closeout, NOT a git hook:
...
# bitty-terminal/bitty-docs-workflow.
#
# What it does:
#   1. Exports `carryctx export --pack-format dir` from this repo into a
#      staging clone of bitty-terminal/bitty-docs-workflow as
#      <UTC-date>-<docs-main-sha>/.
...
#                  <repo>/../bitty-docs-workflow).
#   --url URL      mirror remote for a fresh clone
#                  (default: $WORKFLOW_MIRROR_URL or
#                  https://github.com/bitty-terminal/bitty-docs-workflow.git).
...
SOURCE_REPO="${WORKFLOW_SOURCE_REPO:-bitty-docs}"
